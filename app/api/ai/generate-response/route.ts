...
function getTemplateMessage(context: string, courtier: any): string {
  const templates = {
    'challenge': [
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const {
      courtierNumber,
      context, // 'challenge', 'birthday', 'sale', 'message', 'encouragement', etc.
      referenceMessage, // Message auquel on répond (optionnel)
    ...
    if (!courtier) {
      return NextResponse.json({ error: 'Courtier fictif introuvable' }, { status: 404 });
    }
    ...
    if (isMyBirthday && context !== 'birthday') {
      ...
      // 30% des messages : utiliser DeepSeek
      const prompt = `
...
`;
      ...
      message = getTemplateMessage(context, courtier);
    }
    ...
    return NextResponse.json({
      ...
      courtierName: courtier.firstName,
      usedDeepSeek: !isMyBirthday && shouldUseDeepSeek(),
    });
